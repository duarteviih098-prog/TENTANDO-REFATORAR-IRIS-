...
def ensure_company_storage(empresa_id=None):
    return _legacy().ensure_company_storage(empresa_id=empresa_id)
...
'''
    ...
    + grab(1471, 1480)
    + '\n\n'
    + grab(3240, 3256).replace('ALL_PERMISSIONS.keys()', 'ALL_PERMISSIONS').replace('_get_user_permissions_from_auth', '_get_user_permissions')
    + '\n\n'
    + grab(3292, 3317)
    + '\n\n'
    + grab(3468, 3512),
...
decorators_body = grab(1458, 1469) + grab(1482, 1543)
decorators_body = decorators_body.replace('app.logger.exception', 'current_app.logger.exception')
...
'''
    + grab(3332, 3365)
    + '\n\n'
    + strip_routes(grab(3368, 3429)),
    encoding='utf-8',
)

routes_body = strip_routes(grab(3319, 3321)) + '\n\n' + strip_post(strip_routes(grab(3514, 4029)))
routes_body = routes_body.replace('app.logger.exception', 'app_logger().exception')
...
from app.auth.decorators import ensure_logged_in, require_permission
...
legacy_new = legacy_new.replace('_get_user_permissions_from_auth(user)', '_get_user_permissions(user)')
legacy_new = re.sub(
    r'\ndef _get_user_permissions_from_auth\(user\):[\s\S]*?return \[\]\n\n',
    '\n',
    legacy_new,
    count=1,
)
