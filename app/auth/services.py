...
_LOGIN_ATTEMPTS = {}  # {ip: {'count': N, 'first_at': timestamp, 'blocked_until': timestamp}}
...
def _login_record_failure(ip):
    now = time.time()
    with _LOGIN_ATTEMPTS_LOCK:
        entry = _LOGIN_ATTEMPTS.get(ip) or {'count': 0, 'first_at': now, 'blocked_until': 0}
        ...
        return entry['count'], entry.get('blocked_until', 0)
